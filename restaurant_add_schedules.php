...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    $var_res_id_restaurant = $var_res_date = $var_res_details = $var_res_max_party =
        "";
    $var_res_time_str = "00:00";
    $var_res_time_end = "00:00";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (
            ...
            $sql =
                "INSERT INTO restaurant_reservation(restaurant_id, reservation_day_hash, reservation_time_start, reservation_time_end, reservation_name, reservation_max_party) VALUES(?,?,?,?,?,?)";
            if ($stmt = $mysql->prepare($sql)) {
                $var_sesssion_id = trim($_SESSION["session_id"]);
                $stmt->bind_param(
                    "ssssss",
                    trim($_SESSION["session_id"]),
                    $var_res_date,
                    $var_res_time_str,
                    $var_res_time_end,
                    $var_res_details,
